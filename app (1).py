...
def cell_4_train_model():
    ...
    model_p.save_model(os.path.join(BASE, 'models', 'prophet_model.json'))
...
def cell_5_before_report():
    ...
    def to_img(fig, w=520, h=300):
        return RLImage(
            io.BytesIO(fig.to_image(format='png', width=w, height=h, scale=2)),
            width=15*cm, height=9*cm
        )
    ...
    with open(os.path.join(BASE, 'models', 'prophet_model.json'), 'r') as _f:
        import json as _json
        from prophet.serialize import model_from_json as _mfj
        pm = _mfj(_json.load(_f))
    # Refit on daily data so history_dates is populated
    _conn_pm = get_connection()
    _df_pm = pd.read_sql("SELECT usage_date, total_carbon_kg FROM cloud_usage", _conn_pm)
    _conn_pm.close()
    ...
    pm = _Prophet(yearly_seasonality=True, weekly_seasonality=True)
    pm.fit(_daily_pm)
...
def run_streamlit_dashboard():
    ...
    .stMetric { background: #fff; border-radius: 8px; padding: 12px; border-left: 4px solid #2E7D32; }
    h1 { color: #1B5E20; }
    h2 { color: #2E7D32; }
    h3 { color: #388E3C; }
    ...
    # =========================================================================
        ...
        def load_prophet():
            pm = Prophet()
            pm.load_model(os.path.join(BASE, 'models', 'prophet_model.json'))
            return pm
